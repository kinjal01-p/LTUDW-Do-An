...
exports.single = id_order => {
    var sql = `select * from store.order where id_order = '${id_order}'`;
    return db.load(sql);
}
...
exports.updateStatus = (id,status) => {
    var sql = `UPDATE store.order SET status = ${status} WHERE (id_order = ${id});`;
    
    return db.load(sql); 
}

exports.deleteOrder = (id) => {
    var sql = `delete from store.order
    where id_order = ${id};`
    return db.load(sql); 

}
exports.deleteProductOrder = (id) => {
    var sql = `delete from store.product_order
    where id_order = ${id};`
    return db.load(sql); 
